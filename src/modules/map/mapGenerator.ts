...
import type { ClusteredTour, TileKey, Tour } from '../tour/helpers'
...
export class MapGeneratorV2 {
  ...
  constructor(zoom = settingsModule.settings.mapZoom) {
  ...
  update(
    ...
    for (let xx = -r; xx <= r; xx++) {
      for (let yy = -r; yy <= r; yy++) {
        ...
        if (tour?.clustered?.has(key)) {
          tourPoints.push(...(tour.clustered.get(key) as GeoPoint[]))
        }
      }
    }
    ...
    for (const key of MapGeneratorV2.tilesCache.keys()) {
      if (!relevantKeys.has(key) && !tour?.clustered?.has(key)) {
